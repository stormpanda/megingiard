...
| :--- | :--- | :--- |
| **1. Cold Launch App on Bottom Screen** | • The application launches in immersive fullscreen (no status or navigation bars) on the bottom display, showing the active MacroPad layout grid.<br>• The system **MediaProjection Consent Dialog** appears on the **Top Screen** (Display 0) only if the active layout's remembered state is set to auto-start (`activeLayout.mirrorAutoStart = true`). | If the consent dialog appears on the bottom screen, `ActivityOptions.setLaunchDisplayId` failed to target Display 0. |
| **2. Trigger Mirroring Manually** | • Swipe open the Quick Menu (swipe inward from edge of the Quick Menu Bar).<br>• Tap the **Start Mirroring (Play)** button in the **Mirror Control Card** at the top.\n• Dismiss the Quick Menu.<br>• The system MediaProjection consent dialog appears on the Top Screen.<br>• Accept consent -> Top screen contents begin mirroring on the bottom screen immediately behind the MacroPad buttons, letterboxed or pillarboxed to fit the 4:3 screen. | Tap anywhere outside the Quick Menu to dismiss it. If the screen is black, verify `SurfaceView` z-ordering (`setZOrderMediaOverlay(true)`) is correct. |
| **3. Cutout Layout Editor (Screen Mirroring Edit Mode)** | • Swipe open the Quick Menu, tap the **Screen Mirroring (Edit)** button in the Mirror Control Card.<br>• The layout editor toolbar appears on the secondary screen.<br>• Tap a cutout to select it -> shows corner handles.<br>• Drag a cutout box -> moves the cutout placement.<br>• Drag a corner handle -> resizes the cutout placement.<br>• Click **Edit Crop** in the toolbar -> launches Crop Selector on the primary screen to adjust source crop bounds.<br>• Click **Done** or **Cancel** on the toolbar to exit. | Cutout boundaries are clamped to avoid overlaps and off-screen placement. Layout editor gestures must be completely blocked outside of Screen Mirroring edit mode. |
| **4. Toggle Freeze Frame** | • Swipe open the Quick Menu, tap the **Freeze (Snowflake/Pause)** button.<br>• The mirror freezes as a static image.<br>• Enter Screen Mirroring edit mode -> moving and resizing cutouts work on the frozen bitmap.<br>• Tap **Unfreeze (Play)** in the card -> live mirroring resumes. | If the app crashes on unfreeze, verify the frozen bitmap `recycle()` is managed safely by `ScreenCaptureManager`. |
| **5. Exit Screen Mirroring Edit Mode** | • Click **Done** or **Cancel** in the layout editor toolbar to exit Screen Mirroring edit mode.<br>• The layout editor closes and the normal mirror presentation resumes. | Run standard button actions to verify they trigger inputs successfully on the zoomed mirror crop. |
...
| Action / Test Step | Expected Behavior / Visual Verification | Failure Recovery / Notes |
| :--- | :--- | :--- |
| **1. Set up Touchpad Button** | • Open the layout editor (Quick Menu → **Edit Layout**).<br>• Add or edit a button.<br>• Map the action to **Mouse** → **FullScreenMouse**.<br>• Save changes and exit the editor. | Ensure layout data is persisted. |
...
| Action / Test Step | Expected Behavior / Visual Verification | Failure Recovery / Notes |
| :--- | :--- | :--- |
| **1. Set up Keyboard Button** | • Open layout editor (Quick Menu → **Edit Layout**).<br>• Add/edit a button and map action to **Keyboard** → **FullScreenKeyboard**.<br>• Choose a layout template (e.g. QWERTZ). Save and exit. | Layout templates populate preset keys automatically. |
...
### 2.5 Quick Menu Bar & Quick Menu

The universal navigation overlay accessed via swipe-to-reveal gestures.

| Action / Test Step | Expected Behavior / Visual Verification | Failure Recovery / Notes |
| :--- | :--- | :--- |
| **1. Reveal Quick Menu Bar** | • A tiny translucent tab ("Quick Menu Bar") is visible locked to the right edge of the bottom display.<br>• Swipe the bar inward -> **Quick Menu** slides in smoothly. | The bar must remain visible on the edge across all tool modes. |
| **2. Profile & Layout Quick Select** | • Quick menu opens.<br>• Tap profile row -> select another profile.<br>• Tap layout row -> select another layout.<br>• The underlying MacroPad layout swaps instantly. | Verify rows scroll horizontally if items exceed screen width. |
| **3. Mirror Control Card** | • Locate the Mirror Control Card at the top of the Quick Menu.<br>• Displays mirroring status and control buttons: Play/Stop, Freeze/Unfreeze, and Screen Mirroring (Edit).<br>• Tap **Freeze** -> mirror freezes.<br>• Tap **Stop** -> mirroring ceases. | Mirror control buttons in the Quick Menu must remain in perfect sync with the capture service state. |
| **4. Scrim Dimming & Dismissal** | • Swipe open Quick Menu.<br>• Tap on the dimmed area (scrim) outside the menu.<br>• The menu slides back out of view, and the screen returns to normal. | Swiping the bar outward or tapping the "Close" button must also dismiss the overlay. |
