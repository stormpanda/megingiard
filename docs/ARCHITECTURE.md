...
1. **Launch Routing via `LaunchTrampolineActivity` and Primary Focus Anchor**:
   ...
   - Focus anchoring is strictly managed through deterministic lifecycle callbacks (`onCreate`, `onResume`, `onNewIntent`, `onTopResumedActivityChanged(true)`, and `LaunchTrampolineActivity`), debounced (250ms), and automatically suppressed while `AppStateManager.isPrivdSetupWizardActive` is true to allow the IME on Display 4 to receive input without background touch interference or IME dismissal on Display 0.
