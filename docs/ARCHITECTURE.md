...
1. **Launch Routing via `LaunchTrampolineActivity` and Primary Focus Anchor**:
   ...
   - Because `MainActivity` applies `FLAG_NOT_FOCUSABLE` (so bottom-screen macro touches never steal window focus from top-screen games), `LaunchTrampolineActivity` immediately triggers `PrimaryFocusAnchorActivity` on `Display.DEFAULT_DISPLAY` (Display 0). This lightweight, translucent anchor activity finishes immediately (<5ms, zero animation), guaranteeing that Android WindowManager and `InputDispatcher` always evaluate Display 0 as `FocusedDisplayId`. This eliminates "Application does not have a focused window" ANR timeouts following a launch, resume, or task reordering while strictly preserving top-screen gamepad macro routing.
   - `MainActivity` is declared with `android:launchMode="singleTask"`. If already running on the bottom screen, Android smoothly brings the existing instance to the foreground on the secondary display without spawning a duplicate task on the top display. `MainActivity` continuously maintains primary focus by invoking `PrimaryFocusAnchorActivity.anchorPrimaryFocus(this)` on cold start (`onCreate`), `onResume()`, `onNewIntent()`, and `onTopResumedActivityChanged(true)`.
   - To prevent ANRs when third-party or system focusable windows (e.g. Odin Assistant, notification panels, or system dialogs) close on Display 4 and leave the window manager with no focused window, `MegingiardAccessibilityService` also triggers debounced anchor dispatch on secondary-display `TYPE_WINDOW_STATE_CHANGED` events and system `focus_change` notifications. All anchor invocations are debounced (250ms) and automatically suppressed while `AppStateManager.isPrivdSetupWizardActive` is true to allow the IME on Display 4 to receive input.
