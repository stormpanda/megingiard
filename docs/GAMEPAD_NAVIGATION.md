...
| `GamepadActionCard` | Clickable action card with title, description, icon, and optional trailing status/value badge or swatch (`actionText` / `actionLeadingContent`). | Full card border glow on focus, D-Pad Center / Button A execution. |
...
| `GamepadSliderCard` | Continuous or stepped float slider with customizable track brush or accent track. Supports optional `fineStep` precision mode. | D-Pad Left/Right adjusts value by `step` (or `fineStep` when holding `L2`); Button A confirms and exits; Button B cancels. Displays persistent in-header toast prompt (`"Hold L2 for precise adjustments"`) during Tier 2 adjustment when `fineStep` is available. |
| `GamepadTextFieldCard` | Collapsible in-deck text input card with virtual keyboard integration (56 dp single-row with quoted value headline; expands with `[ Save ]` CTA on editing). | Accent border glow on focus, expands on Button A / Enter; Button A / Enter / Button B saves and collapses. |
