...
- The Library MUST display all installed applications and games in a scrollable condensed grid with square rounded-corner cards (`16.dp` corner radius). Hidden applications MUST display a `visibility_off` Material Symbol badge icon in the top-right corner of their card (with smooth fade-in/fade-out animation when toggled), and the entire card MUST animatively become partially transparent (`0.6f` opacity) when hidden.
