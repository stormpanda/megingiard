...
- Pressing Gamepad **Y** or **Menu** inside the Library view MUST toggle the Library Action Menu. When expanded, the menu MUST support 4-way D-pad navigation mappings corresponding to the layout buttons: **D-pad LEFT** (Hide/Unhide), **D-pad UP** (Add ROM Folder), and **D-pad DOWN** (Remove ROM Folder, only if custom ROM folders are registered).
...
- Starting a dynamic ROM game MUST invoke the appropriate launcher:
  ...
  - **YuzuLauncher**: Invokes Nintendo Switch games (`.nsp`, `.xci`, `.nca`, `.xcz`, `.nsz`) via `Intent(Intent.ACTION_VIEW)` with `dataAndType(uri, "application/octet-stream")` targeting installed Yuzu-compatible emulators (`org.citron.citron_emu`, `org.yuzu.yuzu_emu`, `org.sudachi.sudachi_emu`, `com.suyu.suyu`) with explicit `EmulationActivity` resolution, SAF document content URI resolution with prefix read grants (`FLAG_GRANT_READ_URI_PERMISSION or FLAG_GRANT_PREFIX_URI_PERMISSION`) and ClipData attachment for emulators requiring ContentResolver access (like Citron), `FLAG_ACTIVITY_CLEAR_TOP` task reset, and StrictMode VmPolicy safety.
...
- **EmulatorDetectionFunnel & Detectors:** Central router singleton (`EmulatorDetectionFunnel.kt`) routing foreground process changes to active detectors (`RetroArchDetector`, `GameNativeDetector`, `Pcsx2AndroidDetector`, `YuzuDetector` for Switch emulators, and `PpssppDetector` for standalone PPSSPP emulators), parsing configuration and log files to track running ROM sessions (`ActiveGameSession`) and trigger automatic companion profile switching.
- **RomLauncher & Registry:** Registry singleton (`RomLauncherRegistry.kt`) housing the extensible `RomLauncher` api, mapping emulator ids to specific implementations: `RetroArchLauncher` (resolving absolute paths and firing target activities with `ROM` and `LIBRETRO` parameters), `GameNativeLauncher` (launching PC games via `app_id` extra parameters), and `YuzuLauncher` (launching Nintendo Switch ROMs via `Intent.ACTION_VIEW` resolving SAF content URIs from scanned tree documents or `SafPathResolver` with ClipData and URI permission grants, targeting installed Yuzu, Sudachi, Suyu, or Citron emulators with `EmulationActivity` resolution).
