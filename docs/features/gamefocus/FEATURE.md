...
- Apps marked as hidden MUST be filtered out from all standard gallery categories (Android Games, Android Apps, All Apps, Recently Used). If an app is hidden AND marked as a Favorite, it MUST continue to appear in the Favorites gallery category, but nowhere else in the gallery.
...
- The Library MUST display all installed applications and games in a scrollable condensed grid with square rounded-corner cards (`16.dp` corner radius). Hidden applications MUST display a `visibility_off` Material Symbol badge icon in the top-right corner of their icon card.
- The bottom of the Library screen MUST feature a dedicated bottom action bar housing an `ExpandableActionsMenu` in the lower-left corner (decreasing the vertical height of the app grid container so grid items do not overlap bottom controls) and subdued touch launch indicator buttons in the lower-right corner.
- Pressing Gamepad **Y** or **Menu** inside the Library view MUST toggle the Library Action Menu, exposing vertical D-pad controls for the currently focused library app: **D-pad UP** (Toggle Favorite), **D-pad RIGHT** (Edit Artwork), **D-pad DOWN** (App Info), and **D-pad LEFT** (Hide/Unhide).
