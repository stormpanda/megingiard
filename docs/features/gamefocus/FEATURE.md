...
- Pressing **Select** / **Menu** (`KEYCODE_BUTTON_SELECT`, `KEYCODE_MENU`) on the launcher screen MUST open an `ExpandableActionsMenu` displaying "Actions" when collapsed and expanding to show a non-fading "Close" action (`iconSymbol = "menu"`) and fading secondary actions (such as "Favorites" with `iconSymbol = "gamepad_up"`).
