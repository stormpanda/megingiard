...
- **EmulatorDetectionFunnel & Detectors:** Central router singleton (`EmulatorDetectionFunnel.kt`) routing foreground process changes to active detectors (`RetroArchDetector`, `GameNativeDetector`, `Pcsx2AndroidDetector`, and `YuzuDetector` for Yuzu-family Switch emulators including Citron, Yuzu, Sudachi, and Suyu), parsing log files over `PrivdClient` to track running ROM sessions (`ActiveGameSession`) and trigger automatic companion profile switching.
- **RomLauncher & Registry:** Registry singleton (`RomLauncherRegistry.kt`) housing the extensible `RomLauncher` api, mapping emulator ids to specific implementations: `RetroArchLauncher` (resolving absolute paths and firing target activities with `ROM` and `LIBRETRO` parameters), `GameNativeLauncher` (launching PC games via `app_id` extra parameters), and `Pcsx2AndroidDetector` / `YuzuDetector`.
