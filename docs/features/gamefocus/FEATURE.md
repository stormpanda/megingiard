...
- Pressing **Select** / **Menu** (`KEYCODE_BUTTON_SELECT`, `KEYCODE_MENU`) on the launcher screen MUST open an `ExpandableActionsMenu` hovering at the bottom-left, displaying "Actions" when collapsed and expanding vertically (to the top) to show a non-fading "Close" action (`iconSymbol = "menu"`) at the lowest position (keeping the position of the "Actions" button) and fading secondary actions sorted from top to bottom ("Favorites" with `iconSymbol = "gamepad_up"`, "App Info" with `iconSymbol = "gamepad_down"`, then "Close").
