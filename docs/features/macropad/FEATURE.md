...
- **Emulator Detection Funnel & ROM-Aware Switching:**
  ...
  - `EmulatorDetectionFunnel` forwards the event to either `RetroArchDetector` (which queries `content_history.lpl` playlist files) or `GameNativeDetector` (which queries `wine_logs/wine.log` logs via `PrivdClient` to parse the active Steam app ID or container game name).
