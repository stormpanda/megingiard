...
- **Emulator Detection Funnel & ROM-Aware Switching:**
  ...
  - When a registered emulator package (e.g. `com.retroarch`, `com.retroarch.aarch64`, `org.retroarch`) enters the foreground, `MegingiardAccessibilityService` routes the package to `EmulatorDetectionFunnel`.
  - `EmulatorDetectionFunnel` routes the event to `RetroArchDetector`, which queries RetroArch's `content_history.lpl` playlist file via `PrivdClient` (`READ_FILE` socket command to `megingiard_privd`).
  - `RetroArchLplParser` parses the most recent game session into an `ActiveGameSession` (`romPath`, `gameTitle`, `systemId`, `coreOrBackend`).
  - `AutoSwitchCoordinator` matches `PadProfile.associatedPackage` against the exact ROM path, `rom.<systemId>.<gameTitle>`, system ID (e.g. `"snes"`, `"n64"`), or base package name.
...
2. **AutoSwitchCoordinator Filtering & Mapping**:
   ...
   - **Focus Collision Guard**: If an integration client is active and has reported a focused game, focus events matching the launcher client's package or, for ROM games (starting with `"rom."`), matching any known emulator/container package (e.g., `"app.gamenative"`, `"com.retroarch"`) are ignored. This prevents launcher focus shifts or emulator container events from overriding the active game profile.
   - **Auto-Deactivation Fallback**: If an integration client is active, but the focused package changes to something other than the client, the game, ROM emulator/container packages, or system ignored packages (e.g., the user switched to Google Chrome), the coordinator automatically deactivates the integration client state in `AppStateManager`.
