...
- **Touch Recording Flow:** When the user taps **"Record Touch"** in the timeline editor, they are prompted with a dialog to choose between **"Tap"** and **"Gesture"** recording (the "Don't show again" checkbox is removed as a choice is always required). Confirming starts the screen mirror on the secondary display via `RecordingMirrorPresentation`. To avoid display/VirtualDisplay conflicts on the AYN Thor dual-screen device, the service temporarily releases/pauses the active main mirror capture session and hides the main `MirrorPresentation` window, then shows `RecordingMirrorPresentation` on the secondary display.
  ...
  - **Gesture Mode:** The user performs a continuous, possibly multi-finger gesture. Recording begins automatically as soon as the first finger touches the screen and ends when no fingers remain on the screen (the pointer set becomes empty). The overlay records relative time offsets, coordinate streams (with logical letterbox coordinate clamping to `0f..1f`), and slot-aware pointer IDs (0..9) into `TouchSample` entries, appending a `MacroStep.TouchPath` step.
...
| `MacroData.kt`                   | Macro data model: `Macro` (with `loopEnabled: Boolean` and `loopPauseMs: Int` fields), `MacroStep` sealed class (`GamepadButtonTap`, `JoystickMove`, `JoystickPath`, `DPadTap`, `TouchTap`, `TouchPath`), `JoystickStick` enum, `TouchSample` and `PathSample` structures |
