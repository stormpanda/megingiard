...
- **Smart Alignment Guides & Snapping (PowerPoint-style)**:
  - While dragging or moving a button, the system evaluates the center coordinates of all other buttons in the layout. Whenever the moving button's center aligns horizontally or vertically with another button's center (within a visual tolerance of 2 px), full-canvas dashed alignment guide lines (accent color at 85% alpha, 1.5 dp stroke, `[8f, 6f]` dash intervals) and center indicator dots/rings render across the bottom-screen `PadCanvas`.
  - A persisted toggle **"Snap to Alignment"** (`MacroPadSettings.buttonAlignmentSnapping` / `KEY_MACROPAD_SNAP_ALIGNMENT`) is accessible in both the Layouts deck and the Move Buttons deck.
  - When enabled, moving buttons magnetically lock onto aligned X and Y center coordinates when within `10 px` (`BUTTON_ALIGNMENT_SNAP_THRESHOLD_PX`).
...
- In use mode, dragging a finger on a trackpoint button translates relative motion depending on the configured **Tracking Mode**, scaled by the configurable **Sensitivity Multiplier** (`0.1×` to `5.0×`, default `1.0×`, baseline `MP_TRACKPOINT_SENSITIVITY = 3f`):
  - **Virtual Physical Mouse**: relative motion is translated into **REL_X / REL_Y mouse events** via `MouseInjector.moveMouse()`.
...
- In the editor, action-specific parameters are encapsulated into dedicated sub-pickers (e.g. `TrackpointPicker`, `KeyboardKeyPicker`, `GamepadButtonPicker`, `FullScreenMousePicker`). For `TrackpointMove`, `TrackpointPicker` provides dedicated cards for **Tracking Mode** (`PHYSICAL_MOUSE` vs `VIRTUAL_TOUCH`), **Trackpoint Size** (`SMALL` / `MEDIUM` / `LARGE`), and **Pointer Sensitivity** (`0.1×` to `5.0×` stepper in `0.1×` increments).
...
- **Macro Creation Workflows (`ChooseMacroMode`)**: When creating a new macro (from Quick Actions or the Macros Deck), the user is presented with a dedicated creation mode picker:
  ...
  - **Type Text Sequence**: Prompts for text input and timing configuration (key duration and pause between keystrokes), automatically generating individual `KeyboardKeyTap` steps for the entire phrase.
...
- Each macro contains a list of **`MacroStep`** subtypes: `KeyboardKeyTap`, `GamepadButtonTap`, `JoystickMove`, `JoystickPath`, `DPadTap`, `TouchTap`, and `TouchPath`. Each step has `startTimeMs` and `durationMs` fields that allow overlapping parallel steps within the same macro.
  - **`KeyboardKeyTap`**: Injects Linux keycodes (1 to 464, including `KEY_FN`) via `KeyInjector`. Supports up to 2 optional modifier keys (Ctrl, Alt, Shift, Meta, Fn). Modifiers are pressed before the base key and released after it in reverse order. `VisualKeyboardPicker` is integrated in the step editor for picking keys visually.
  - `JoystickPath` is created exclusively by the physical gamepad recorder and carries a list of timestamped `PathSample` entries that replay the full continuous stick trajectory.
  - `TouchPath` is created by the continuous gesture recorder and carries a list of timestamped `TouchSample` entries that replay a full continuous multi-touch trajectory.
...
- **Stuck-Key Safety & Event Compiler Ordering**:
  - `MacroExecutor` tracks all pressed buttons and keys (`pressedButtons`, `pressedKeys`). If a macro is cancelled or stopped prematurely, its `finally` block executes and immediately releases all active keys and buttons.
  - The event compiler marks key/button releases with `isReset = true` so key releases at timestamp $T$ are processed before key presses at timestamp $T$, preventing dropped keystrokes during rapid repeats.
...
- Users can duplicate existing layouts to reuse their configuration, buttons, and cutouts (deep copy with new UUIDs). Layouts can also be copied to another profile, which duplicates referenced profile macros when necessary and displays a confirmation toast. Individual buttons can also be copied to another layout across any profile, which duplicates referenced profile macros if cross-profile and displays a confirmation toast upon success.
...
### FR-P9c: Per-Layout Background Touchpad
...
### FR-P9d: SteamGridDB Image Scraper
...
### FR-P9e: Background & Mask Image Dimming

- Users can dim the background image or mask overlay image using a `GamepadSliderCard` in their respective layout settings editors (`LayoutBackgroundSubPageContent`, `LayoutMaskSubPageContent`).
...
### FR-P9f: Background & Mask Image Scaling Modes

- Users can select how the background image and mask overlay image scale to fit the secondary display's aspect ratio via a `GamepadChoiceCard` in the respective layout settings editors (`LayoutBackgroundSubPageContent`, `LayoutMaskSubPageContent`).
...
- Persisted in `PadLayout.bgScaleMode` and `PadLayout.maskScaleMode` (`BackgroundScaleMode`), defaulting to `FILL` for seamless backward compatibility.
...
- **Editor Integration & Custom Labeling**: App Launcher buttons include the **Label** text input field in `PadButtonEditDialog`, allowing full custom naming (e.g. for accessibility, list views, and delete confirmation). When an application is chosen from the app picker, the button label is automatically populated with the application's resolved name if the existing label was blank or default. The **Icon** picker remains hidden as the app's launcher icon is loaded dynamically from `PackageManager`.
...
| --- | --- |
| `selectedSection: StateFlow<EditorSection>` | Active primary category deck (`QUICK_ACTIONS`, `PROFILES`, `LAYOUTS`, `AUTOMATION`, `MIRROR`, `BACKGROUND`, `BUTTONS`, `MACROS`). |
...
- **Left Category Sidebar (210 dp):** Features category navigation tiles with illuminated focus borders and L1/R1 bumper tab switching:
  ...
  - **Buttons (`SmartButton`):** Add Button trigger and reorderable button list with focus indicators, edit, duplicate, copy to layout (with confirmation toast), and delete actions.
...
- **Auto Switch Reactivation Prompt on Editor Exit:** If Automatic Profile Switching was active (`CompanionViewMode.AUTO`) when opening the MacroPad editor and is subsequently deactivated during the editor session (e.g. by creating a new profile which sets `companionViewMode` to `MACROPAD`, or by manually switching modes), `AppStateManager.wasAutoSwitchDeactivatedInEditor` tracks this transition. When the user exits the editor (via Gamepad `B` button at the root stack, header close button `✕`, or outer scrim click), `PrimaryOverlayContainer` closes right away so no dialogs are stacked on top of each other. Once the primary overlay dismisses, `PrimaryModalHost` dispatches a standalone top-screen modal (`PrimaryModalType.REACTIVATE_AUTO_SWITCH`) via `GamepadConfirmModal`:
  - **Visual Alignment:** The dialog visually mirrors `PrimaryOverlayContainer`'s design tokens: a structured header bar (`colors.surfaceVariant.copy(alpha = 0.8f)`, 56 dp height, 20 dp padding, 18 sp SemiBold title in `onSurface`, accent icon, and header close button `✕`), content area matching the editor deck background (`colors.appBackground`), and card border framed with `rememberBezelBrush()` on a 16 dp rounded surface with 16 dp shadow elevation. Focus is strictly trapped within the card (`focusGroup()`, `focusProperties { exit = { FocusRequester.Cancel } }`).
  - **Reactivate Action:** Primary card (**Reactivate Auto Switch** / `Icons.Rounded.AutoFixHigh`, auto-focused by default, description "Restore automatic profile and layout switching") restores `CompanionViewMode.AUTO`, clears the tracking flag, and closes the modal.
  - **Dismiss Action:** Secondary card (**Keep Auto Switch Off** / `Icons.Rounded.Close`, description "Remain in manual profile mode") leaves the companion in manual mode, clears the tracking flag, and closes the modal.
  - **Gamepad B / Cancel:** Pressing Gamepad `B`, system Back, clicking the backdrop scrim, or tapping the header close button `✕` immediately closes the dialog without reactivating auto switch, clearing the tracking flag and keeping the user's manual mode intact.
...
| `MacroPadEditor.kt`              | Full-screen layout editor built on `GamepadTwoPaneScaffold`: orchestrates the 9 sidebar categories (Quick Actions, Profiles, Layouts, Automation, Mirror, Background, Mask, Buttons, Macros), deck content columns, in-deck sub-page stack navigation, and exit auto-switch reactivation prompt (`GamepadConfirmModal`). |
| `AutomationDeck.kt`              | Automation category deck content (`AutomationDeckContent`): profile-level automatic layout switching toggle, anchor calibration overview, active layout live presence status badges (`PRESENT` / `LOST`), and direct entry to anchor configuration. |
...
| `AlignmentMath.kt`               | Pure Kotlin math helper in `:shared:core`: generalized center snapping, button adapters (`calculateButtonAlignmentSnap`, `calculateGamepadButtonMove`, `findAlignedCenterGuides`), cutout adapters, visual guide line discovery, and grid snap algorithms (`snapPosition`, `snapRectangular`, `snapRadial`) |
| `PadCanvas.kt`                   | Bottom-screen live editor pad canvas: button drag positioning, smart alignment guides overlay (`AlignmentGuidesOverlay`), grid overlay rendering (`GridMode`, `GridOverlay`), highlight border when unlocked, directional drag handles when unlocked, inward-pointing triangle indicators (`HighlightPointer`) for highlighted buttons when locked, animated lock badge on lock/unlock transitions; accepts `layout: PadLayout?`, `gridMode`, and `isLocked` parameters |
...
| `BackgroundSettingsEditor.kt`    | In-deck content (`LayoutBackgroundSubPageContent`) for background image selection, live bottom-screen preview, in-place active/inactive crop toggle, scaling mode, dimming, and `GamepadSaveExitActionRow` save controls. |
| `MaskSettingsEditor.kt`          | In-deck content (`LayoutMaskSubPageContent`) for mask overlay image selection, live bottom-screen preview, in-place active/inactive crop toggle, scaling mode, dimming, and `GamepadSaveExitActionRow` save controls. |
