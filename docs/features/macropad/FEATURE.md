...
- **Visual Action Pickers (`VisualKeyboardPicker`, `VisualGamepadPicker`, `VisualMousePicker`, `ActionGridSubPages`)**:
  ...
  - Modifiers (Mod 1, Mod 2 for keyboard), combo buttons (Extra 1, Extra 2, Extra 3 for gamepad), and specific app target (for App Launcher) remain configurable directly on the `EditButton` page below or as the primary action card.
...
MacroTimelineEditor (Composable in-deck sub-page, opened from Macros Deck)
  ├── SubPage header: macro name card
...
- **Navigation & Sub-Pages:** Managed through a clean `subPageStack: List<MacroPadSubPage>` backstack. Sub-pages (QuickActions, ButtonEdit, Background, Touchpad, MacroTimeline, MacroStepEdit, ColorWheel, AppPicker, Reorder, Copy) render directly in unified `GamepadDeck` instances with automatic breadcrumbs (`'PARENT  ›  SUBPAGE'`) and focusable controls. Supports deep-linking into specific sections/sub-pages via `PrimaryModalPayload`.
- **In-App Deep-Linking & Modal Suspension/Resumption:**
  ...
  - During Touch Macro Recording, `MacroTimelineEditor` calls `AppStateManager.suspendCurrentAndDismiss()` to close the editor overlay while recording on the mirrored canvas, and upon recording finish/cancellation `TouchRecordingOverlay` calls `AppStateManager.resumeSuspended()` to immediately restore the timeline editor at the recorded macro step.
...
| `MacroPadEditor.kt`              | Full-screen layout editor built on `GamepadTwoPaneScaffold`: orchestrates the 4 sidebar categories (Profiles, Layouts, Buttons, Macros), deck content columns, and in-deck sub-page stack navigation.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `QuickActionsSubPage.kt`         | In-deck sub-page (`QuickActionsSubPageContent`) presenting deep-link navigation shortcuts to frequently used layout tools and global preferences.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `PadCanvas.kt`                   | Bottom-screen live editor pad canvas: button drag positioning, grid overlay rendering (`GridMode`, `GridOverlay`), highlight border when unlocked, animated lock badge on lock/unlock transitions, snap functions (`snapRectangular`, `snapRadial`); accepts `layout: PadLayout?`, `gridMode`, and `isLocked` parameters |
...
| `TouchRecordingManager.kt`       | Domain singleton coordinating both touch-tap and touch-gesture recording flows: `requestRecording(mode)` → `recordingMode`, `recordingRequested`, and `state` (`Idle / Recording / Done`) StateFlows → `MainAppScreen` shows `TouchRecordingOverlay` → tap mode emits one `recordedTap`; gesture mode accumulates completed gesture segments as separate `MacroStep.TouchPath` steps via `recordGestureCompleted(samples, startOffsetMs)`; `finishRecording()` sorts steps, trims the leading idle offset so the first gesture starts at `t = 0`, and emits `TouchRecordingState.Done`; `cancelRecording()` discards all accumulated segments                                                                                                                                                                                                                                                                                                                                                                                      |
| `MacroTimelineEditor.kt`         | Single-macro step timeline editor: in-deck sub-page (`MacroTimelineSubPageContent`) orchestrating List/Timeline views, undo/redo state, shift settings, loop settings, and touch/gamepad recording flows.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `MacroStepEditDialog.kt`         | In-deck sub-page (`MacroStepEditSubPageContent`) for creating/editing a single `MacroStep`; `TouchTap` and `TouchPath` steps show recorded coordinates or paths read-only with editable start/duration timing                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `TouchRecordingOverlay.kt`       | Compose overlay rendered over `EmbeddedMirrorView` during touch recording; `TapCaptureOverlay` captures the first in-bounds tap and injects a 50 ms feedback touch; `GestureCaptureOverlay` records continuous multi-finger gestures and injects them live — out-of-bounds coordinates for already-tracked pointers are clamped rather than dropped; `TouchRecordingControls` renders Cancel / Stop & Save in the lower letterbox band; all overlays update `TouchRecordingManager`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
