...
| `MacroPadEditor.kt`              | Full-screen layout editor built on `GamepadTwoPaneScaffold`: orchestrates the 8 sidebar categories (Quick Actions, Profiles, Layouts, Automation, Mirror, Background, Buttons, Macros), deck content columns, in-deck sub-page stack navigation, and exit auto-switch reactivation prompt (`GamepadConfirmModal`). |
