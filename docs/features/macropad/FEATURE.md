...
- **Gamepad recording flow:** The user taps **"Record Gamepad"** in the timeline editor → a confirmation dialog appears (skipped on subsequent uses via `SettingsManager.skipGamepadRecordDialog` — activated by the "Don't show again" button in the dialog) → the user confirms → `MacroTimelineEditor` starts `GamepadInjector`, calls `GamepadRecordingManager.startRecording()`, and renders `GamepadRecordingOverlay` inline above the editor → touches on face buttons, D-Pad, shoulder buttons (LB/LT on left, RT/RB on right), Start/Select, and both stick circles (with L3/R3) are simultaneously (a) forwarded live to the primary display through `GamepadInjector` and (b) recorded as `MacroStep.GamepadButtonTap`, `MacroStep.DPadTap`, and `MacroStep.JoystickMove` events → tapping **Stop** finalises the recording, trims the idle offset before the first input to 0 ms, and appends the recorded step block to the current macro timeline. The **Home** button (`BTN_MODE`) is not exposed in the recording overlay.
- **Gamepad recording overlay layout:** Four-zone symmetric layout — (1) Title bar (title + Abbrechen/Fertig); (2) Controls strip: a single full-width row using `Arrangement.SpaceBetween` with three groups — left `[LB (outer) | LT (inner)]`, center `[SE | ST]` with small gap, right `[RT (inner) | RB (outer)]`; (3) Main area rendered as `BoxWithConstraints` with absolute proportional positioning: L-Stick center at 16 %/35 %, R-Stick at 84 %/35 %, D-Pad bottom edge at 96 % vertical (at 26 % horizontal), Face Cluster bottom edge at 96 % vertical (at 74 % horizontal) (all as fraction of BoxWithConstraints width/height); positions are clamped with `coerceAtMost`/`coerceAtLeast` to prevent off-screen clipping; `GRO_FACE_CLUSTER_SIZE = 176 dp` ensures 6 dp gap between adjacent face buttons; (4) Bottom strip `Row(SpaceBetween)`: L3 (left edge) and R3 (right edge) at equal padding. The internal `PressableSurface` composable uses `pointerInput(Unit)` with `rememberUpdatedState` for its callbacks — this prevents gesture interruption caused by lambda-key instability when state changes trigger a recomposition mid-press.
...
| `MacroData.kt`                   | Macro data model: `Macro`, `MacroStep` sealed class (`GamepadButtonTap`, `JoystickMove`, `DPadTap`, `TouchTap`), `JoystickStick` enum                                                                                                                   |
| `MacroExecutor.kt`               | Fire-and-forget macro playback: compiles steps to sorted event list, replays with coroutine delays; starts/stops `TouchInjector` automatically when any `TouchTap` step is present                                                                      |
...
| `MacroTimelineEditor.kt`         | Single-macro step timeline editor: visual Canvas timeline + step list; "Record Touch" chip triggers recording flow and appends `TouchTap` step via `LaunchedEffect(recordedTap)`                                                                        |
