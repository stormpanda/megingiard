...
- **Gamepad recording overlay layout:** Four-zone symmetric layout — (1) Title bar (title + Abbrechen/Fertig); (2) Controls strip: a single full-width row using `Arrangement.SpaceBetween` with three groups — left `[LB (outer) | LT (inner)]`, center `[SE | ST]` with small gap, right `[RT (inner) | RB (outer)]`; (3) Main area — two equal `weight(1f)` columns, each using `Arrangement.SpaceBetween`: left column has L-Stick at top and D-Pad at bottom, right column has R-Stick at top and Face-Button cluster at bottom; both sticks and both bottom clusters are at the same vertical level; all sticks use the same `GRO_STICK_SIZE = 144 dp`; (4) Bottom strip: L3 at the left edge and R3 at the right edge using `Arrangement.SpaceBetween` with equal horizontal padding.
