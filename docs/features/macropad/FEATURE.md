...
- The grid mode cycles **Off → Rectangular → Radial → Off** via a single toggle button located in the editor toolbar row (to the right of the "Button" chip), above the canvas preview.
...
- Macros are managed via the **Macro Library** editor (opened via a dedicated "PlaylistPlay" action button row below the profile chips, or a "Macros…" item in the profile options dropdown menu).
...
- Macros are managed at the profile level. The profiles section displays a dedicated **"Macros…"** (PlaylistPlay) action button row below the profile chips, and also embeds a **"Macros…"** shortcut as the first action inside the profile's dropdown options menu.
