...
- **Gamepad recording overlay layout:** Four-zone symmetric layout — (1) Title bar (title + Abbrechen/Fertig); (2) Controls strip: a single full-width row using `Arrangement.SpaceBetween` with three groups — left `[LB (outer) | LT (inner)]`, center `[SE | ST]` with small gap, right `[RT (inner) | RB (outer)]`; (3) Main area rendered as `BoxWithConstraints` with absolute proportional positioning: L-Stick center at 16 %/35 %, R-Stick at 84 %/35 %, D-Pad center at 26 %/76 %, Face Cluster center at 74 %/76 % (all as fraction of BoxWithConstraints width/height); positions are clamped with `coerceAtMost`/`coerceAtLeast` to prevent off-screen clipping; `GRO_FACE_CLUSTER_SIZE = 176 dp` ensures 6 dp gap between adjacent face buttons; (4) Bottom strip `Row(SpaceBetween)`: L3 (left edge) and R3 (right edge) at equal padding.
