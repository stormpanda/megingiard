...
- Macros support timing randomization (`Macro.randomizeTimingEnabled = true`). When enabled, a precalculated random timing offset in `[0, randomizeTimingRangeMs]` (inclusive) is dynamically generated and added to the configured start time of each step on every execution run of the macro. The maximum random range is configured via a slider that ranges from 10 to 100 ms (default 10 ms). The numbers must be freshly generated on each execution run rather than being a one-time addition.
