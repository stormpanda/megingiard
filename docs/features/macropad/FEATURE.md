...
- Macros support timing randomization (`Macro.randomizeTimingEnabled = true`). When enabled, random timing offsets in `[0, randomizeTimingRangeMs]` (inclusive) are dynamically generated and added to both the configured start time and the duration of each step on every execution run of the macro. Two separate random numbers are generated for each step. The maximum random range is configured via a slider that ranges from 10 to 100 ms (default 20 ms). The numbers must be freshly generated on each execution run rather than being a one-time addition.
...
- **Profile Association:** Each `PadProfile` stores an optional `associatedPackage: String?` mapping representing the Android package name of the target application. This field is nullable and defaults to `null` (no mapping).
...
2. **AutoSwitchCoordinator Filtering & Mapping**:
   ...
   - If `SettingsManager.autoSwitchProfiles.value` is true, it searches for a profile in `MacroPadState.profiles` where `it.associatedPackage` matches the package name (case-insensitive).
...
`MacroPadEditor` is rendered as a full-screen in-tree overlay (`Box` inside the same composition), controlled by UI state in the hosting screen. No separate `Dialog` window is created — this is intentional so that the editor works correctly both in the main `Activity` and inside `MirrorPresentation` (secondary display), where `AlertDialog`/`Dialog` would crash with `BadTokenException` due to a null window token. All confirmation and name-input overlays inside `MacroPadEditor` (delete button, delete profile, rename profile, new profile, new layout, edit layout) follow the same pattern: in-tree `Box` composables (`InlineConfirmDeleteOverlay`, `InlineNameInputOverlay`, `NewLayoutOverlay`, `InlineLayoutSettingsOverlay`) instead of `AlertDialog`. Profile-level settings (shape, size) are also available directly in `MacroPadToolSettings` without opening the full editor. The editor features horizontally scrollable chip rows for both profile and layout selection (`EditorProfileChipsBar` and `EditorLayoutChipsBar`). Next to each row of chips is an Edit button that opens the corresponding settings dialog. Deletion has been moved into the headers of both the profile and layout edit dialogs, aligned on the right-hand side. If a profile or layout is the only one in existence, it cannot be deleted; the delete button in the dialog header is disabled and styled with `0.38f` alpha. The Add button (plus icon) has been moved into the right-hand side of the Profile and Layout section header separators, colored with the active accent color. Layout chips support drag-reordering via long press. When a layout is disabled (hidden), `(hidden)` is appended to its chip text. The editor list is scrollable via `LazyColumn`.
...
**Toggle** — A small `IconButton` placed in the editor toolbar row (to the right of the "Add Button" and "Macros…" chips, above the canvas) cycles the grid mode. Icons: `GridOff` (off), `Grid4x4` (rectangular), `TripOrigin` (radial). The button uses a rounded-rectangle border that matches the chip style; the icon tint is accent-coloured when a grid is active and secondary-surface-coloured when off.
