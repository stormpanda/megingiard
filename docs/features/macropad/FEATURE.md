...
| `MacroExecutor.kt`               | Tap-to-toggle macro playback: compiles steps to sorted event list, replays with coroutine delays; tracks one `Job` per macro ID via `ConcurrentHashMap`; exposes `runningMacroIds: StateFlow<Set<String>>` for UI reactivity; `stop(macroId)` cancels the job; supports `Macro.loopEnabled` — loops the event sequence until stopped, with an optional `loopPauseMs` delay between iterations; starts/stops `TouchInjector`/`GamepadInjector` automatically in a `try/finally` block |
