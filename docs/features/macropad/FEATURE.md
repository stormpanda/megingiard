...
- Macros support timing randomization (`Macro.randomizeTimingEnabled = true`). When enabled, precalculated random timing offsets in `[0, randomizeTimingRangeMs]` (inclusive) are dynamically generated and added to both the configured start time and the duration of each step on every execution run of the macro. Two separate random numbers are generated for each step. The maximum random range is configured via a slider that ranges from 10 to 100 ms (default 20 ms). The numbers must be freshly generated on each execution run rather than being a one-time addition.
