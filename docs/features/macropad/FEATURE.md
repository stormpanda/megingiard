...
- **Strict Privileged Mode Guarding**: The entire macro subsystem (creation, editing, physical gamepad recording, screen touch recording, test runs, and use-mode button execution) is strictly guarded behind Privileged Mode (`megingiard_privd` active and `PrivdClient.isConnected == true`). When Privileged Mode is inactive:
  - In use-mode, Macro buttons render with disabled visual styling (hatched/semi-transparent) via `MacroPadHitTestEngine.isDeviceDisabled()`, and tapping them triggers a localized floating feedback banner (`macropad_device_disabled_macro_privd`).
  - In the MacroPad Editor, the Macros Deck displays an informative `GamepadInfoBox` warning banner (`macropad_macro_privd_required_banner`), and attempting to create a new macro displays a feedback toast.
