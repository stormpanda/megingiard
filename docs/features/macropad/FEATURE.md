...
- **Visual Action Pickers (`VisualKeyboardPicker`, `VisualGamepadPicker`, `VisualMousePicker`, `ActionGridSubPages`)**:
  - In the button editor, selecting the primary action card opens a dedicated visual sub-page picker:
    ...
    - **Overlay Action Picker**: Displays all 3 overlay/launcher actions (App Launcher, Fullscreen Mouse, Fullscreen Keyboard) in a 2-column card grid.
    - **Layout Action Picker**: Displays all 3 layout actions (Next Layout, Previous Layout, Profile Switcher) in a 2-column card grid.
  - Modifiers (Mod 1, Mod 2 for keyboard), combo buttons (Extra 1, Extra 2, Extra 3 for gamepad), and specific app target (for App Launcher) remain configurable directly on the `EditButton` page below the primary action card.
