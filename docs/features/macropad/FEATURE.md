...
- **Per-layout button color defaults** (`PadLayout.buttonTextColor` / `PadLayout.buttonBorderColor` / `PadLayout.buttonBgColor`): Each layout can independently configure the default colors for text/icon, border, and fading color.
  ...
  - Configured through dedicated sub-menus in the gamepad navigation hierarchy:
    ...
    - **Swords / Button Preview & Live Bottom-Screen In-Flight Preview**: Both the color picker wheel and sub-page action cards display preview buttons with live-rendered text, border, and background styles. Furthermore, all in-flight color adjustments (Neutral, Accent, Custom, or slider drags in the Color Wheel) are streamed immediately to the secondary bottom display (`PadCanvas`) in real time via transient preview state (`MacroPadState.previewLayout` / `setPreviewButton`), showing the exact visual result without requiring the user to save settings first. Discarding or navigating back immediately reverts the bottom screen to the saved baseline state, while saving commits the changes permanently to DataStore.
