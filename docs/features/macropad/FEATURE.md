...
| `ButtonAlignmentMath.kt`         | Pure Kotlin math helper in `:shared:core`: button center alignment calculations (`calculateButtonAlignmentSnap`, `calculateGamepadButtonMove`, `findAlignedCenterGuides`), visual guide line discovery, and grid snap algorithms (`snapPosition`, `snapRectangular`, `snapRadial`) |
