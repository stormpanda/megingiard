...
- **Slider Preview Mode:** Sliders in the settings overlay support a live-preview mode (activated by tapping the eye icon next to a slider). In preview mode, a bottom-anchored slider bar is rendered directly over the live ambient screen on the secondary display. Touch input on the secondary display is restricted exclusively to the active preview slider and its cancel/confirm buttons — all other screen areas are blocked from touch interaction, the edge menu pill is completely hidden, and edge-swipe gesture processing is disabled. Touch input on the primary display is blocked entirely.
