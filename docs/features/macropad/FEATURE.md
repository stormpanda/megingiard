...
- **Gamepad recording overlay layout:** Three-row layout — Row 1: shoulder buttons only (LB+LT left-aligned, RT+RB right-aligned, icon-only); Row 2: left stick + L3 button (left-aligned) and face button cluster (right-aligned); Row 3: D-Pad (left), Select+Start column (center, bottom-aligned), right stick + R3 button (right-aligned).
