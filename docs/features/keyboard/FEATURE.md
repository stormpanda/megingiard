...
- Focusing a non-editable element or navigating away from the current application package to a different app/launcher on the primary display MUST automatically dismiss the virtual keyboard. Transient window transitions, suggestion dropdowns, or popup dialogs within the same active package MUST NOT dismiss the virtual keyboard.
- If the user manually collapses or dismisses the virtual keyboard while an editable field remains focused, the system MUST record hysteresis (`userDismissedFieldId`, using stable `AccessibilityNodeInfo.uniqueId`) to avoid aggressively re-opening the keyboard until focus changes to another field or the user explicitly taps the field again.
