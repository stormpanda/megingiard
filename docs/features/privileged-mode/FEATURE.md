...
- Clicking the button MUST evaluate device setup conditions and run the appropriate automated pipeline on Display 0 via `MegingiardAccessibilityService`:
  ...
  - **Language-Independent Navigation**: By utilizing standardized Android system view Resource IDs (such as `com.android.settings:id/switch_widget`, `android:id/switch_widget`, `com.android.settings:id/switch_bar`, `com.android.settings:id/main_switch`, `android:id/title`, and dialog buttons like `android:id/button1`), settings traversal and toggling logic is inherently language-independent. The service prioritizes actual leaf switch widgets (`switch_widget`, `switch_bar`) over container bar layouts (`main_switch`) and dispatches click actions whenever the system Wireless Debugging state is inactive (even if a container/switch UI node reports checked due to stale OS state). The locale configuration (`AutoSetupLanguageConfig`) is used for mapping target keyword text checks (e.g. pairing dialog title matches or system warning confirmations) across German, Spanish, French, English, and Traditional Chinese (`zh-TW`, `zh-HK`, `zh-MO`) system locales. If a targeted settings element is off-screen (such as the Build number row at the bottom of About Phone), the service MUST recursively find scrollable containers and execute scroll forward actions to bring them into view.
