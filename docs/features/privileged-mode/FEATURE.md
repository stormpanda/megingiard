...
- Clicking the button MUST evaluate device setup conditions and run the appropriate automated pipeline on Display 0 via `MegingiardAccessibilityService`:
  ...
  - **Multi-Language System Navigation**: Settings search queries and UI node matching MUST dynamically adapt to the Android system language (e.g. English vs. German `"Build-Nummer"`, `"Debugging über WLAN"`, `"IP-Adresse & Port"`) by querying `LocaleManager.systemLocales` via `AutoSetupLanguageConfig`.
