...
- Clicking the button MUST evaluate device setup conditions and run the appropriate automated pipeline on Display 0 via `MegingiardAccessibilityService`:
  ...
  - **Full-Service Auto-Connect**: Upon completing pairing, the service automatically initiates `PrivdManager.connect()` to start the privileged daemon seamlessly without extra taps.
