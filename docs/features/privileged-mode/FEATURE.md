...
- Clicking the button MUST evaluate device setup conditions and run the appropriate automated pipeline on Display 0 via `MegingiardAccessibilityService`:
  ...
  - **Full-Service Auto-Connect**: Upon completing pairing, the service waits for the pairing dialog to dismiss and for `adbd` keys database to stabilize (using a 1500ms delay), rescans the screen for the connect port, and automatically initiates `PrivdManager.connect()` to start the privileged daemon seamlessly without extra taps.
