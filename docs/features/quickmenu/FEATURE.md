...
- **Show Companion Hub / Show MacroPad / Auto View Mode** — rendered as full-width action chips at the bottom card. When the MacroPad grid is currently visible, it displays "Show Companion Hub" (`R.string.quick_menu_show_dashboard`) and tapping it sets `companionViewMode = CompanionViewMode.DASHBOARD`. When the Companion Hub is currently visible, it displays "Show MacroPad" (`R.string.quick_menu_show_macropad`) and tapping it sets `companionViewMode = CompanionViewMode.MACROPAD`. Whenever `companionViewMode` is explicitly set to `DASHBOARD` or `MACROPAD`, an "Auto View Mode" (`R.string.quick_menu_auto_mode`) chip is also displayed above it, allowing the user to easily revert to `CompanionViewMode.AUTO`.
