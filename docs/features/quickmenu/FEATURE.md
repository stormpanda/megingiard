...
- **Show Companion Hub / Show MacroPad & Auto Mode Toggle** — rendered as a side-by-side action row at the bottom card. The primary manual action button displays "Show Companion Hub" (`R.string.quick_menu_show_dashboard`) when MacroPad is visible, and "Show MacroPad" (`R.string.quick_menu_show_macropad`) when Companion Hub is visible. Tapping it sets `companionViewMode` to `DASHBOARD` or `MACROPAD`, which locks the view mode and disables Auto Mode. Next to it, an "Auto" (`R.string.quick_menu_auto_mode`) chip is displayed as a sticky toggle. When Auto Mode is active (`companionViewMode = CompanionViewMode.AUTO`), the chip is illuminated with the primary accent color; tapping it freezes the current view mode and pauses profile auto-matching. When Auto Mode is inactive, tapping the chip re-enables `CompanionViewMode.AUTO` and immediately re-evaluates the foreground app/emulator focus to match profiles and views dynamically.
