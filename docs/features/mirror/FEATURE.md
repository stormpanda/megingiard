...
- **1px Outset Borders & Clean Interior:** To prevent borders from obscuring or bleeding into the mirrored video stream (especially on small or flat cutouts during adjustment), borders on both displays are rendered with a 1px width (`1.dp`) strictly on the **outside perimeter (outset)** of the selection/crop area:
  - On the primary display (`CropSelectorOverlay`), the border is rendered outward into the surrounding dark scrim (`offset(-1.dp, -1.dp)` and `size(+2.dp, +2.dp)`). This guarantees that 0 border pixels are drawn inside the captured region, completely eliminating border capture into the mirror video stream.
  - On the secondary display (`CutoutLayoutEditor`), both selected and unselected cutouts render their 1px border outside the cutout bounds with a 100% transparent interior, ensuring zero occlusion of the mirrored video content.
