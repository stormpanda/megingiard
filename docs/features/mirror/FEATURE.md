...
- **Automated HUD Isolation via Pixel-Level Color Change (`HudAutoTuneCoordinator`, `HudAutoTuner`, `CutoutMaskManager`)**:
  ...
  - **Automated Commit & Mask Persistence:** Upon 6-second completion, the generated mask bitmap is saved under `context.filesDir/cutout_masks/mask_<cutoutId>.png` and the raw per-pixel variance map under `mask_<cutoutId>_var.bin` by `CutoutMaskManager`. The cutout is updated with `hasTransparencyMask = true`, `maskFeathering = 0`, `maskTranslucency = 0`, and `AppStateManager.resumeSuspended()` automatically reopens `MacroPadEditor` on Display 0 right back to the Cutout Settings subpage with the auto-tuned mask active.
  - If a stationary scene is detected (player did not move in-game), the tuner warns with a helpful hint toast.
  - **HUD Translucency Slider:** When a transparency mask is active (`cutout.hasTransparencyMask`), a dedicated "HUD Translucency" slider (`0%` / Off to `100%`, step `5%`) is displayed above the feathering slider in the cutout settings deck. Semi-transparent HUD elements (such as Genshin Impact skill buttons, translucent minimap centers, status dials, and glass panels) shift in color over moving 3D scenery, but physically dampen background variance ($\Delta C = (1 - \alpha) \cdot \Delta B$). The engine applies two color-agnostic recovery mechanisms:
    1. *Geometric Enclosure Infill with Soft Barrier:* A 2D border-connected flood fill detects cavities enclosed by HUD borders. A soft barrier threshold ($14 + \text{translucency} \times 0.55$) allows semi-transparent boundary rings (e.g. dial circles) to seal the interior cavity, relaxing allowed variance inside enclosed dials up to $30 + \text{translucency} \times 1.20$ (up to $150$).
    2. *Proximity Halo Relaxation:* Multi-source distance expansion from solid core anchors relaxes allowed variance within an expanded $4 \dots 48\text{ px}$ radius (up to $179$ near anchors at 100%) with distance-based alpha blending.
    Because the raw 8-bit variance map is saved alongside the mask, adjustments update live at 60 FPS without requiring a 6-second recalibration.
  ...
  - **HUD Absence Detection & Freeze-Frame During Cutscenes (`HudPresenceManager`, `HudPresenceEvaluator`):**
    - When a game enters a cutscene, dialogue sequence, inventory menu, or loading screen, the game engine disables its HUD layer (`HUD.visibility = false`). Without detection, the static cutout coordinates continue mirroring the primary display, projecting moving 3D scenery, camera pans, or video playback through the transparency mask stencil.
    - **Anchor Signature Matching:** During HUD Auto-Tuning, an `HudAnchorSignature` is computed by sampling up to 48 stratified anchor points across an $8 \times 8$ spatial grid where variance $\le 10$, recording their normalized coordinates $(u, v)$ and expected reference RGB colors. Alongside the mask, the signature is saved to `mask_<cutoutId>_anchor.json` and the initial high-res calibration frame crop to `mask_<cutoutId>_freeze.png`.
    - **Lightweight 4 Hz Evaluation:** When mirroring is active and cutouts have `freezeOnHudLoss == true`, `HudPresenceManager` samples downscaled frames (960x540) at 4 Hz. `HudPresenceEvaluator` measures the anchor match ratio ($\Delta E \le 45$ RGB tolerance) with a 2-check hysteresis state machine to prevent single-frame flickering.
    - **Seamless Freeze Frame Rendering:** When the HUD is detected as lost, `MultiCutoutContainer` suppresses the live cutscene video and renders the most recent valid HUD frame (or the calibrated reference frame from disk) scaled to the cutout bounds, with the transparency mask stencil applied on top. The user sees their button graphic perfectly preserved, stable, and readable throughout cutscenes without any background video leakage. As soon as the cutscene ends and the HUD returns, live 60 FPS mirroring resumes instantly.
    - **Toggle Control:** The user can enable or disable this behavior per cutout via the "Freeze during Cutscenes" toggle (`freezeOnHudLoss`) in the cutout settings editor.
  - **Clear Mask Action:** When a mask is active (`cutout.hasTransparencyMask`), the user can clear it at any time via a dedicated "Clear Transparency Mask" action card, which unloads the mask, deletes mask PNG, variance map, anchor signature, and freeze frame from disk, and resets `maskFeathering`, `maskTranslucency`, and `freezeOnHudLoss` to `0` / `false`.
- **Zero-Configuration UI:**
  - The cutout settings editor contains exclusively the [ Auto-Tune HUD ] button, the [ HUD Translucency ] slider, the [ HUD Edge Feathering ] slider, the [ Freeze during Cutscenes ] toggle (when a mask is present), and the [ Clear Transparency Mask ] button, avoiding complex manual keying formulas.
