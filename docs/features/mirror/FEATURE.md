...
- **Dynamic Live Frame Buffering & Animated Blur Freezing (`HudPresenceManager`, `MultiCutoutContainer`):**
  ...
  - When the anchor element disappears (match ratio $< 45\%$) or manual freeze is engaged, the cutout immediately engages a frosted-glass blur-in animation ($0\text{px} \to 8\text{px}$ over 350 ms) using `AccelerateDecelerateInterpolator()` for a smooth, natural defocus transition that perfectly balances with the recovery dissolve, displaying the latest valid live frame.
