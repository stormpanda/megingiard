...
- **Layout Anchor Calibration (`VisualAutoTuneCoordinator`):**
  ...
  - **Color Diversity Tiebreaker:** During anchor point extraction across the 8×8 grid, candidate pixels with identical minimum temporal variance (e.g. $v = 0$) prioritize colors that maximize Manhattan distance from already-selected anchor points rather than defaulting to the first pixel in raster-scan order. This prevents dominant background colors (such as solid white padding) from saturating the 64 sample points, ensuring the signature captures high-contrast icons and text to eliminate false-positive matches during monochromatic transitions (e.g. full-white loading screens).
