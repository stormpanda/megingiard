...
- **Profile-Level Automatic Layout Switching (`PadProfile.autoLayoutSwitching`, `AnchorPresenceManager`):**
  ...
  - When enabled and autonomous mode is active (`CompanionViewMode.AUTO`), `AnchorPresenceManager` manages dynamic layout transitions across all calibrated anchored layouts within the active profile:
    - While the active layout's anchor is `PRESENT`, monitoring runs at ~60 Hz with zero candidate probing overhead.
      - **16-Point Deterministic Sparse Probing:** In the steady `PRESENT` state (0 consecutive mismatches), `AnchorPresenceManager` executes `AnchorPresenceEvaluator.matchesSparseProbe`, evaluating a deterministic 16-point subgrid (every 4th sample across the 64 points) against the master frame. If all 16 points match within color tolerance, the anchor remains `PRESENT` without reading the remaining 48 pixels, slashing pixel reads by 75% while preserving continuous 60 Hz ring-buffer stream delivery and 0 ms transition detection latency. On the very first diverging pixel, it immediately falls back to a full 64-point ratio evaluation.
