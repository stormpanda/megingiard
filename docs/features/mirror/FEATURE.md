...
- **Top-Screen Anchor Positioning (`AnchorSelectorOverlay`):**
  ...
  - **1% Minimum Anchor Size:** Anchor bounding boxes can shrink down to 1% of screen dimensions (`MIN_ANCHOR_SIZE = 0.01f`, ~19×11 px on 1080p, matching standard cutout limits), enabling pinpoint anchoring on compact in-game UI badges and tiny HUD icons (e.g. small inventory bag icons).
  - Positioning directly updates `layout.visualAnchor` (`srcX`, `srcY`, `srcWidth`, `srcHeight`).
  - **Immediate Post-Positioning Calibration Prompt:** Confirming anchor position presents an `AppAlertDialog` confirmation dialog asking *"Calibrate Anchor Now?"* with **[Calibrate]** (atomically launches layout anchor calibration without requiring navigation through settings) and **[Later]** (returns to Layout Settings), preventing uncalibrated anchors from being forgotten.
