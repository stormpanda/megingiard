...
- **Advanced Cutout Controls (Calibrated State):**
  ...
  - **Cutout Translucency Slider:** A dedicated slider (`0%` / Off to `100%`, step `5%`) allowing real-time adjustment of the allowed color variance threshold to recover semi-transparent elements, floating sparkles, glass backplates, and glowing icons without distance constraints.
...
- Mask state is persisted per-cutout in `ScreenCutout` (`hasTransparencyMask: Boolean`, `maskTranslucency: Int = 0`, `maskSensitivity: Int = 14`, `maskCavityHealing: Boolean = true`, `maskAlphaMatting: Boolean = false`, `renderAsStaticAsset: Boolean = false`).
