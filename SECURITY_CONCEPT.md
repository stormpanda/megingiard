...
- A rogue process claiming the local TCP port before the real daemon.
...
| Privd mutual HMAC authentication  | Authenticate both app and daemon before privileged commands are accepted.                     | `CHAL/AUTH/OK/VERIFY/PROOF` over local TCP socket loopback, using HMAC-SHA256 with a per-install 32-byte key encrypted at rest in Android Keystore (AES-256-GCM). The key is never embedded in the APK. See **Per-install key scheme** below. | [Privileged Mode](docs/features/privileged-mode/FEATURE.md#security-model)                                                            |
| Privd security boundary           | Cryptographically verify that only the provisioned app instance can connect to the daemon.    | Both sides perform mutual challenge-response with the per-install private key. Rejects rogue apps (mismatched key) and spoofed daemons.                                                                                                | [Privileged Mode](docs/features/privileged-mode/FEATURE.md#security-model)                                                            |
...
| Rogue app tries to connect to the daemon                   | Daemon rejects the connection if HMAC handshake verification fails           |
| Rogue process binds the socket first, impersonating daemon | App rejects the connection if mutual HMAC verification proof fails           |
