...
object SteamGridDbClient {
    ...
    suspend fun searchGames(query: String, apiKey: String): Result<List<SteamGridDbGame>> {
        ...
        return fetchString(urlString, apiKey).mapCatching { jsonStr ->
    ...
    suspend fun fetchImages(gameId: Int, type: String, apiKey: String): Result<List<SteamGridDbImage>> {
    ...
    suspend fun downloadImageToTempFile(imageUrl: String, cacheDir: File): Result<File> = withContext(Dispatchers.IO) {
    ...
    private suspend fun fetchString(urlString: String, apiKey: String): Result<String> = withContext(Dispatchers.IO) {
        var connection: HttpURLConnection? = null
        try {
